...
export default class ContactCard extends Component {
  ...
  componentDidMount(){
    getContactInfo("tempUserName", "87826582").once("value").then((snapshot) => {
      this.setState({
        name: snapshot.val().name,
        company: snapshot.val().company,
        email: snapshot.val().email,
        phone: snapshot.val().phone,
        websites: snapshot.val().websites,
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <View className="websites">
          {this.displayInfo(this.state.websites)}
        </View>
