  ...
  getList() {
    ...
    getContactInfo(this.props.data.user.uid).on("value", (snapshot) => {
      if(snapshot.val()){
        ...
        let followUpList = [];
        keys.map((key) => {
          let { name, company, contactID, followUp} = dbContactList[key].info;
          if(followUp){
            contactList.push(<View style={{width: Dimensions.get('window').width, height: 30, backgroundColor: 'magenta'}}><Text style={this.checkFollow(followUp)}>{name} | {company}</Text></View>);
          }
          else {
            followUpList.push(<View style={{width: 100, height: 30}}><Text>{name} | {company}</Text></View>);
          }
        });
        let finalList = contactList.concat(followUpList);
        this.setState({
